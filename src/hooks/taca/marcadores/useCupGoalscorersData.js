...
/**
 * Custom hook to fetch and process CUP goalscorers data
 * Only counts goals from Cup matches (competition_type = 'Cup')
 *
 * @param {number} seasonId - Selected season ID
 * @returns {Object} - { goalscorers, teams, loading }
 */
export const useCupGoalscorersData = (seasonId) => {
  ...
  useEffect(() => {
    ...
    const fetchCupGoalscorers = async () => {
      setLoading(true);

      // Fetch ONLY CUP matches for selected season
      const { data: matches, error: matchesError } = await supabase
        .from("matches")
        .select("id")
        .eq("competition_type", "Cup") // ← Only Cup matches
        .eq("season", seasonId);

      if (matchesError) {
        console.error("Error fetching cup matches:", matchesError);
        setLoading(false);
        return;
      }
      ...
      const fetchAllMatchEvents = async () => {
        ...
        while (hasMore) {
          const { data: matchEvents, error } = await supabase
            ...
            .eq("event_type", 1) // event_type = 1 is Goal
